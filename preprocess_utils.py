"""
预说明：
本代码大部分函数取自split_and_recognition.py，用于单独预处理代码，得到 28*28 字符块
以此训练CNN模型
"""
...
def perspective_warp_plate(img: np.ndarray, vertices: np.ndarray, target_size=(240, 80)) -> np.ndarray:
    """
    透视变换车牌：根据四角顶点裁成矩形车牌，返回 rectified BGR 图
    """
    def order_vertices(pts):
    ...
    W, H = target_size
    try:
        src_ordered = order_vertices(vertices)
...
def parse_vertices_from_filename(filename: str) -> np.ndarray:
    """
    从 CCPD 文件名解析四个顶点并返回顶点数组
    """
    ...
    if len(vertex_strs) != 4:
        raise ValueError(f"Expected 4 vertices, got {len(vertex_strs)} in {filename}")

    # 提取四个点 (x,y)
    pts = []
    for s in vertex_strs:
        x_str, y_str = s.split(',')
...
def protect_leftmost_char(img, candidate_x, crop_top=0, crop_bottom=None, sample_width=3, sample_ratio=0.6, margin_ratio=0.2, gray_thresh=20):
    """
    检查最左侧候选裁剪线是否会误切汉字，如果左右颜色相近则取消裁剪，直接拿左边线作为监测边界
    返回修正后的最左裁剪线，如果保护汉字则返回0，否则返回candidate_x
    """
    h, w = img.shape[:2]
...
def detect_long_lines(bin_img, orientation='h', edge_ratio=0.110, angle_tol=15, length_ratio=0.4):
    ...
    detected 为 True 表示通过梯度检测到线（标红），False 表示使用默认边界线（标蓝）
    """
    h, w = bin_img.shape
    if orientation == 'h':
...
def vertical_projection_split_2(plate_bw: np.ndarray, expected_chars: int,
                              output_path: Optional[pathlib.Path] = None,
                              gap_radius: float = 0.02) -> List[Tuple[int,int]]:
    """
    剔除空隙后左右分别均分字符
    自动根据 expected_chars 选择 gap_pos 和分割方式：
      - 7位普通车牌：gap_pos=0.32，左侧2字，右侧5字
      - 8位新能源车牌：gap_pos=0.29，左侧2字，右侧6字
    """
    h, w = plate_bw.shape
    splits = []
    ...
    if gap_pos is not None:
        ...
        last = 0
        if left_count > 0:
            left_split = [i * gap_left // left_count for i in range(1, left_count+1)]
            for x in left_split:
                splits.append((last, x))
                last = x

        # 右侧
        right_split = [gap_right + i * (w - gap_right) // right_count for i in range(1, right_count+1)]
        last = gap_right
        ...
        # 均分整个宽度
        last = 0
        for i in range(1, expected_chars+1):
            x = i * w // expected_chars
            splits.append((last, x))
            last = x
