...
save_path = os.path.join(file_path, 'point')
...
# 训练和测试时将数据移到 GPU：inputs, targets = inputs.to(device), targets.to(device)
...
def test(epoch):
    ...
    if not os.path.isdir(save_path):  # 使用配置的 save_path
        os.mkdir(save_path)
    torch.save(state, os.path.join(save_path, 'ckpt_%d_acc_%f.pth' % (epoch, acc)))
